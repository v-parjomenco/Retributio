    ...
    class EntityManager {
      public:
        ...
        [[nodiscard]] Entity create() {
            ...
            if (!mFreeList.empty()) {
                ...
                if (id >= mSparse.size()) {
                    mSparse.resize(
                        id + 1,
                        kInvalidIndex); // меняем размер, заполняя пустые элементы kInvalidIndex
                }
            }

            const std::size_t denseIndex =
                mDense.size(); // denseIndex - индекс сущности в плотном массиве, т.е.
                               // место, в конце вектора mDense, куда добавляем новую сущность
            mDense.push_back(Entity{id});
            mSparse[id] = static_cast<EntityId>(
                denseIndex); // добавляем id сущности, которую положили mDense в mSparse
        ...
        [[nodiscard]] bool isAlive(Entity e) const noexcept {
            ...
            if (denseIndex ==
                kInvalidIndex) { // Если там kInvalidIndex, значит сейчас нет такого живого элемента
                return false;
            }
            ...
            return denseIndex < mDense.size() && mDense[denseIndex].id == id;
        }
