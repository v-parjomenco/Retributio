...
namespace core::utils {
    ...
    inline void showMessageBox(const std::string& text, const std::string& title, UINT type) {
        MessageBoxW(nullptr, utf8ToNative(text).c_str(), utf8ToNative(title).c_str(),
    ...
    namespace message {
        ...
        inline void logDebug([[maybe_unused]] const std::string& message) {
            ...
            std::cout << ansi::cyan << "[Отладка] " << ansi::blue << message << ansi::reset
                      << std::endl;
#endif
#endif
        }
...
#define DEBUG_MSG(msg)                                                                             \
    ::core::utils::message::logDebug(std::string("[") + __FILE__ + ":" +                           \
