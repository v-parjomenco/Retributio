...
struct PlayerConfig {
    ...
    std::string anchor = config::DEFAULT_ANCHOR;    // базовый якорь, используется для задачи стартовой позиции
    sf::Vector2f startPosition{ 0.f, 0.f };         // стартовая позиция, используется только если якорь не задан
