...
namespace core {
    ...
    PlayerConfig ConfigLoader::loadPlayerConfig(const std::string& path) {
        ...
        PlayerConfig cfg; // создаёт объект конфигурации игрока со значениями по по умолчанию из config.h
        ...
        if (data.contains(Player::CONTROLS) && data.at(Player::CONTROLS).is_object()) {
            ...
            cfg.controls.up = json_utils::parseKey(c, Player::CONTROL_UP, cfg.controls.up);
            cfg.controls.down = json_utils::parseKey(c, Player::CONTROL_DOWN, cfg.controls.down);
            cfg.controls.left = json_utils::parseKey(c, Player::CONTROL_LEFT, cfg.controls.left);
            cfg.controls.right = json_utils::parseKey(c, Player::CONTROL_RIGHT, cfg.controls.right);
        }
        else {
            utils::message::showError("[ConfigLoader]\nБлок controls отсутствует или имеет неверный тип. "
        ...
        if (core::anchors::fromString(cfg.anchor) == core::AnchorType::None &&
            cfg.anchor != "None") {
            utils::message::showError(
