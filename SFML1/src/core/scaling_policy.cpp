...
namespace core {
    ...
    void UniformScalingPolicy::apply(sf::Sprite& sprite, const sf::View& view) {
        ...
        if (mLastUniform > 0.f) // проверка, чтобы избежать деления на 0
            ratio = newUniform / mLastUniform; // соотношение, насколько изменилось окно по сравнению с прошлым разом

        sf::Vector2f currentScale = sprite.getScale(); // получаем текущий коэффициент масштабирования спрайта
        sprite.setScale({ currentScale.x * ratio, currentScale.y * ratio }); // перемножаем старый коэффициент с новым
