...
sf::Color parseColor(const json& value, const sf::Color& fallback) {
    if (value.is_string()) {
        ...
        if (!s.empty() && s[0] == '#') {
            ...
            try {
                if (s.size() == 6) {
                    return { hexTo(s.substr(0,2)), hexTo(s.substr(2,2)), hexTo(s.substr(4,2)), 255 };
                } else if (s.size() == 8) {
                    return { hexTo(s.substr(0,2)), hexTo(s.substr(2,2)), hexTo(s.substr(4,2)), hexTo(s.substr(6,2)) };
                }
...
DebugOverlayConfig loadDebugOverlayConfig(const std::string& path) {
    ...
    try {
        core::utils::json::JsonValidator::validate(data, {
            {"enabled",        {json::value_t::boolean}, false},
            {"position",       {json::value_t::array, json::value_t::object, json::value_t::number_float}, false},
            {"characterSize",  {json::value_t::number_integer, json::value_t::number_unsigned}, false},
    ...
    cfg.characterSize = static_cast<unsigned>(data.value("characterSize", static_cast<int>(cfg.characterSize)));
    if (data.contains("color")) cfg.color = parseColor(data["color"], cfg.color);
