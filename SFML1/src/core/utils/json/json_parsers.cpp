...
namespace {
    ...
    parseColorFromValueWithIssue(const json& value, const sf::Color& fallback) noexcept {
        ...
        if (value.is_object()) {
            ...
            auto parseChannel =
                [&](const char* field, Issue::Channel ch, std::uint8_t& dst) -> bool {
            ...
            if (!parseChannel("r", Ch::R, rr) ||
                !parseChannel("g", Ch::G, gg) ||
                !parseChannel("b", Ch::B, bb) ||
                !parseChannel("a", Ch::A, aa)) {
                return r;
            }
    ...
    parseVec2fFromValueWithIssue(const json& value, const sf::Vector2f& defaultValue) noexcept {
        ...
        if (value.is_object()) {
            ...
            if (const auto xIt = value.find("x"); xIt != value.end()) {
                if (!xIt->is_number()) {
                ...
                float fx = 0.f;
                if (!core::utils::json::detail::tryGetFiniteFloat(*xIt, fx)) {
                    r.issue.kind = Vec2ParseIssue::Kind::NumberNotFiniteOrOutOfRange;
                    r.issue.axis = Vec2ParseIssue::Axis::X;
                    r.issue.elementIndex = kInvalidIndex;
                    return r;
            ...
            if (const auto yIt = value.find("y"); yIt != value.end()) {
                if (!yIt->is_number()) {
                    r.issue.kind = Vec2ParseIssue::Kind::ObjectFieldNotNumber;
                ...
                float fy = 0.f;
                if (!core::utils::json::detail::tryGetFiniteFloat(*yIt, fy)) {
                    r.issue.kind = Vec2ParseIssue::Kind::NumberNotFiniteOrOutOfRange;
                    r.issue.axis = Vec2ParseIssue::Axis::Y;
                    r.issue.elementIndex = kInvalidIndex;
                    return r;
    ...
    parseFloatFromValueWithIssue(const json& v,
                                 float defaultValue,
                                 float minValue,
                                 float maxValue,
...
namespace core::utils::json::detail {
    ...
    [[nodiscard]] bool tryGetFiniteFloat(const json& v, float& out) noexcept {
        ...
        const long double minF =
            static_cast<long double>(std::numeric_limits<float>::lowest());
        const long double maxF =
        ...
        if (const auto* pf = v.get_ptr<const FloatT*>()) {
            ...
            if (d < static_cast<FloatT>(minF) || d > static_cast<FloatT>(maxF)) {
                return false;
            }
...
namespace core::utils::json {
    ...
    std::string_view describe(const Vec2ParseIssue& issue) noexcept {
        ...
        case Kind::ArrayElementNotNumber:
            return (issue.elementIndex == 0)
                ? "элемент массива [0] не число"
                : "элемент массива [1] не число";
        case Kind::ObjectFieldNotNumber:
            switch (issue.axis) {
            case Vec2ParseIssue::Axis::X: return "поле объекта 'x' не число";
            case Vec2ParseIssue::Axis::Y: return "поле объекта 'y' не число";
            default: return "поле объекта не число";
            }
    ...
    Vec2ParseResult parseVec2fWithIssue(const json& data,
                                        std::string_view key,
        ...
        return ::parseVec2fFromValueWithIssue(*it, defaultValue);
    }
    ...
    std::string_view describe(const KeyParseIssue& issue) noexcept {
        ...
        case Kind::MissingKey: return "ключ отсутствует";
        case Kind::WrongType: return "ожидалась строка";
        case Kind::UnknownName: return "неизвестное имя клавиши";
        default: return "неизвестная ошибка";
        }
    }

    KeyParseResult parseKeyWithIssue(const json& data,
                                     std::string_view key,
        ...
        const sf::Keyboard::Key parsed = parseKeyString(name);
    ...
    std::string_view describe(const ColorParseIssue& issue) noexcept {
        ...
        case Kind::InvalidHex:
            ...
            case Ch::R: return "неверные hex-цифры канала R";
            case Ch::G: return "неверные hex-цифры канала G";
            case Ch::B: return "неверные hex-цифры канала B";
            case Ch::A: return "неверные hex-цифры канала A";
            default: return "неверные hex-цифры";
            }
        case Kind::ObjectFieldNotNumber:
            switch (issue.channel) {
            case Ch::R: return "поле 'r' не число";
            case Ch::G: return "поле 'g' не число";
            case Ch::B: return "поле 'b' не число";
            case Ch::A: return "поле 'a' не число";
            default: return "поле объекта не число";
            }
        case Kind::ObjectFieldOutOfRange:
            switch (issue.channel) {
            case Ch::R: return "поле 'r' вне диапазона 0..255";
            case Ch::G: return "поле 'g' вне диапазона 0..255";
            case Ch::B: return "поле 'b' вне диапазона 0..255";
            case Ch::A: return "поле 'a' вне диапазона 0..255";
            default: return "поле объекта вне диапазона 0..255";
            }
        default:
            return "неизвестная ошибка";
        }
    ...
    ColorParseResult parseColorWithIssue(const json& data,
                                         std::string_view key,
        ...
        return parseColorFromValueWithIssue(*it, defaultValue);
    }
    ...
    std::string_view describe(const UnsignedParseIssue& issue) noexcept {
        ...
        case Kind::None: return "OK";
        case Kind::MissingKey: return "ключ отсутствует";
        case Kind::WrongType: return "ожидалось целое число";
        case Kind::Negative: return "значение отрицательное";
        case Kind::OutOfRange: return "значение вне диапазона unsigned";
        default: return "неизвестная ошибка";
        }
    }

    UnsignedParseResult parseUnsignedWithIssue(const json& data,
                                               std::string_view key,
        ...
        return ::parseUnsignedFromValueWithIssue(*it, defaultValue);
    }

    FloatParseResult parseFloatWithIssue(const json& data,
                                         std::string_view key,
        ...
        return ::parseFloatFromValueWithIssue(*it, defaultValue, 0.f, 0.f, false);
    }
    ...
    FloatParseResult parseFloatWithIssue(const json& data,
        ...
        return ::parseFloatFromValueWithIssue(*it, defaultValue, minValue, maxValue, true);
    }
    ...
    std::string_view describe(const FloatParseIssue& issue) noexcept {
        ...
        case Kind::None: return "OK";
        case Kind::MissingKey: return "ключ отсутствует";
        case Kind::WrongType: return "ожидалось число";
        case Kind::NotFiniteOrOutOfRange:
            return "значение не конечное или вне диапазона float";
        case Kind::OutOfRange: return "значение вне допустимого диапазона";
        default: return "неизвестная ошибка";
        }
    }
