...
namespace {
    ...
    [[nodiscard]] std::size_t checkedMulSizeT(
        ...
        if (a != 0u && b > (std::numeric_limits<std::size_t>::max() / a)) {
            LOG_PANIC(core::log::cat::ECS,
                      "SpatialIndexV2(Atrapacielos): {} overflow (a={}, b={})",
                      context, a, b);
        }
        return a * b;
    }

    [[nodiscard]] std::uint32_t computeExpectedMaxEntitiesAtrapacielosFromActiveSet(
        const std::size_t activeSetCeiling) {

        if (activeSetCeiling == 0u) {
            LOG_PANIC(core::log::cat::ECS,
                      "SpatialIndexV2(Atrapacielos): invalid active-set ceiling (0)");
        }
        ...
        if (cap > static_cast<std::size_t>(std::numeric_limits<std::uint32_t>::max())) {
            LOG_PANIC(core::log::cat::ECS,
                      "SpatialIndexV2(Atrapacielos): expected maxEntityId {} exceeds uint32 range",
                      cap);
        }
...
namespace game::atrapacielos::config {
    ...
    std::size_t computeStableIdCapacityAtrapacielos(
        ...
        // В Atrapacielos текущая "реальность" ≈ peak активного набора,
    ...
    core::ecs::SpatialIndexSystemConfig buildSpatialIndexV2ConfigAtrapacielos(
        ...
        // Базовые бюджеты Atrapacielos (Debug/Release).
        ...
        if (maxEntitiesPerChunkCeiling == 0u) {
            ...
            if (base == 0u) {
                LOG_PANIC(core::log::cat::ECS,
                          "SpatialIndexV2(Atrapacielos): invalid budgets (visible=0 and dirty=0)");
            }
        ...
        if (activeSetCeiling > (std::numeric_limits<std::size_t>::max() - nonStreamingHeadroom)) {
            LOG_PANIC(core::log::cat::ECS,
                      "SpatialIndexV2(Atrapacielos): activeSetCeiling + headroom overflow "
        ...
        if (rawOverflowMaxNodes >
            ...
            LOG_PANIC(core::log::cat::ECS,
                      "SpatialIndexV2(Atrapacielos): overflow.maxNodes {} exceeds uint32 range",
                      rawOverflowMaxNodes);
        }
        ...
        if (shiftBudget64 <= 0 ||
            ...
            LOG_PANIC(core::log::cat::ECS,
                      "SpatialIndexV2(Atrapacielos): shiftBudget {} invalid (window {}x{})",
        ...
        LOG_INFO(core::log::cat::ECS,
                 "SpatialIndexV2(Atrapacielos): display {}x{} px, viewport {}x{} chunks, "
