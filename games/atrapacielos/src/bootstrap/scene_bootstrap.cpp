...
namespace {
    ...
    using PlayerBlueprint = game::atrapacielos::config::blueprints::PlayerBlueprint;
    ...
    void resolvePlayerSpriteDerived(ResourceManager& resources,
        ...
        // Политика Atrapacielos: textureRect = full texture, origin = center.
...
namespace game::atrapacielos::bootstrap {
    ...
    SceneBootstrapResult preloadAndResolveInitialScene(ResourceManager& resources,
        ...
        // Atrapacielos поддерживает максимум 2 игрока (архитектурное ограничение MVP).
        // Лучше ловить это на границе сцены, до прелоада и спавна.
        if (config.players.size() > 2) {
            LOG_PANIC(core::log::cat::Gameplay,
                      "[{}] Too many player blueprints ({}). Atrapacielos supports max 2 players.",
        ...
        result.backgroundKey =
            resolveTextureOrPanic(resources, "atrapacielos.background.desert", kWhere);
        ...
        result.stressPlayerKey =
            resolveTextureOrPanic(resources, "atrapacielos.sprite.player", kWhere);
        pushUnique(textures, textureCount, *result.stressPlayerKey, kWhere);
#endif

        // Preload сцены (textures/fonts). Sounds в Atrapacielos пока не используются.
