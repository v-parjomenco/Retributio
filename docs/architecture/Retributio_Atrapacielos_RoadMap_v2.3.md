# Retributio / Atrapacielos → 4X "Auctoritas"
...
### **Atrapacielos (Short-term, 6 months)**
...
### **Game Layer**
- ✅ Atrapacielos = real-time arcade (separate from core)
- ✅ Auctoritas = turn-based simulation (separate from core)
- ✅ Shared core, different contracts

### **Dev-Only**
- ✅ Stress/soak/harness scenes ONLY under `RETRIBUTIO_PROFILE`
...
### Deliverables:
- [ ] Repository layout (`core/`, ``, `game/Auctoritas/`, `assets/`)
...
- [ ] Compile-time gates (`RETRIBUTIO_PROFILE`, `NDEBUG`)
...
**Goal:** ECS architecture + determinism core + Atrapacielos prototype in parallel.
...
### **Week 1-2: Atrapacielos Prototype (Minimal, Dirty)**
...
- [ ] **Centralized RNG:**
  ...
  ```
- [ ] **Physics Separation:**
  - Atrapacielos arcade physics → `float` allowed (not deterministic, ok for real-time)
  - Auctoritas simulation core → `fixed-point` or deterministic math libraries (future)
...
**Why:** Atrapacielos (arcade) doesn't stress 4X risks. Harness fills gap.
...
- [ ] **Config File Watcher:**
  ...
  // Atrapacielos example:
  mConfigWatcher.watch("assets/config/player.json",
      []() { reloadPlayerStats(); });
  
  // Auctoritas example (future):
  mConfigWatcher.watch("assets/game/Auctoritas/config/provinces.json",
      []() { reloadProvinceData(); });
  ```
- [ ] **Universal Design:** Works for all games (Atrapacielos + Auctoritas)
...
## **PHASE 3 — Atrapacielos MVP (Complete Game)** (Months 3-6)
...
### **Month 4: 1v1 Multiplayer (Atrapacielos Dogfight Mode)**
...
- **State Synchronization:** Send position/rotation/velocity 20 times/sec + interpolation
  ...
  - Lockstep reserved for Auctoritas (turn-based)
...
## 🚀 **RELEASE: Atrapacielos on Steam (Month 6)**

**Price:** $5-10  
**Revenue Goal:** $10K-50K → fund Auctoritas development  
**Platform:** Steam (Windows initially, Linux/Mac via CMake builds)

---

## **PHASE 4 — Auctoritas Foundation** (Months 7-12)
...
- [ ] **OPTIONAL: Texture Array Backend:**
  ```cpp
  #ifdef RETRIBUTIO_USE_TEXTURE_ARRAYS
...
- [ ] **Mod Loading:**
  ```
  assets/        # Base game data
  assets/game/Auctoritas/  # Auctoritas-specific data
...
## **PHASE 5 — Auctoritas Core Gameplay** (Months 13-24)
...
- **ECS UI FOR:**
  ...
  - Simple buttons/panels (Atrapacielos-style)
...
## 🚀 **EARLY ACCESS RELEASE: Auctoritas (Month 33)**
...
- [ ] Codebase clean for Auctoritas (no hacks in `core/`)
- [ ] $10K-50K Steam revenue

---

### **Auctoritas Prototype (Month 24)**
...
- ❌ **NO 30 FPS ANYWHERE** (60 FPS absolute floor, 144 FPS target, 240 FPS Atrapacielos)
