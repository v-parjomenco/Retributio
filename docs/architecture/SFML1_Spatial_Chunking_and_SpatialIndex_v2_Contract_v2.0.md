# SFML1 — Spatial Chunking & SpatialIndex v2 Contract (SkyGuard + Titan) — v2.0
Approved: 26.01.2026

Status: **Approved, commit-ready architecture contract**  
Audience: engine/core + game layers (SkyGuard today, Titan later)  
Language: C++20, performance-first; **Titan is determinism-first**.
...
- **TileMap chunking contract** for Titan-style tile worlds (future), without polluting SkyGuard.

It avoids Titan gameplay/balance configs and does not assume a Titan executable exists today.  
...
- **World units**: canonical unit for simulation/spatial queries.
  - SkyGuard: simulation may remain `float`, but partitioning must be robust for large coordinates.
  - Titan: world units are **pixels** (`tileSizePx = 256`) in simulation.
...
- **World chunk**: coarse partition of world space identified by `ChunkCoord {x,y}`.
  ...
  - World may be unbounded (SkyGuard) while using fixed-size chunks.
...
- Provide a **headless 4X-scale harness** (Debug/Profile only) to validate risks without building Titan gameplay.
...
- Must support both:
  - bounded flat chunk addressing (Titan),
  - bounded-capacity sparse active window (SkyGuard).

### 2.3 TileMap contracts (Titan)
...
### 2.5 Practicality gate (SkyGuard)
If a feature is Titan-critical but SkyGuard cannot meaningfully validate it, we do:
- interface + invariants + profiler hooks now,
- implementation later (Titan prototype phase).

No invented APIs with no validation path.

---

## 3. Determinism & numeric representation (Titan)

Titan is turn-based and requires deterministic replays. Simulation must not depend on:
- nondeterministic iteration order from hash containers,
- platform-dependent floating-point behavior.

### 3.1 Titan world-space representation
Contract:
- Titan simulation uses **integer pixel coordinates** (`int32_t`) for positions and AABBs where feasible.
- Rendering uses float coordinates **relative to camera** (presentation only).

AABB contract (Titan simulation):
...
- Titan integer AABB semantics must not be mixed with SkyGuard float AABB semantics.

SkyGuard may keep float AABBs; Titan does not.
...
- stable output ordering by a deterministic key (**Titan uses StableID, never `entt::entity`**).

Deterministic mode must be hard to misuse:
- API exposes separate methods: `queryFast()` and `queryDeterministic()`.
- In Titan Debug: calling `queryFast()` from simulation scope must assert/fail.
- In Titan Profile/Release: calling `queryFast()` from simulation scope must be a deterministic fail-fast (log critical + terminate).

### 3.3 StableID subsystem (mandatory for Titan)

- Titan requires `StableId = uint64_t`, monotonically increasing and never reused.
...
- StableID assignment order must be deterministic:
  - Titan must enforce deterministic entity creation order (no spawning from unordered iteration without sorting by deterministic keys).

SkyGuard:
- StableID service may be present but is not required; SkyGuard does not use deterministic query mode.

### 3.4 Deterministic ordering mechanism (Titan, zero-overhead)

Titan uses **stable traversal + deterministic insertion** (no per-query sorting):
...
`SFML1/include/core/spatial/chunk_coord.h`  
...
`WorldPos` is a small POD chosen by the game layer (float for SkyGuard; int32 pixels for Titan).
...
- Deterministic iteration in deterministic mode (Titan).
- One algorithm usable by SkyGuard and Titan via specialized storage backends.
...
- Titan builds require a 32-bit entity backend:
  - `static_assert(sizeof(entt::entity) == 4, "Titan requires 32-bit entt::entity backend");`
...
Notes:
- Civ-style tile stacks must not force large cell densities. Titan may store stack membership in gameplay/tile data; SpatialIndex density stays low for most cells.
...
- insertion order into overflow must be deterministic (Titan: order by StableID, or deterministic update order + stable per-cell insertion policy).
...
Overflow pool is bounded by a configurable ceiling (Titan) or budget (SkyGuard).
...
- Deferred destruction is mandatory for Titan safety and deterministic replays.


## 9. Updates: dirty entities only (deterministic in Titan)
...
Titan deterministic update order:
...
### 10.1 Titan bounded maps: flat addressing + payload pooling
...
### 10.2 SkyGuard unbounded scrolling: bounded-capacity active window (specialization)

SkyGuard is unbounded in -Y, but the active window is small and camera-follow driven.
...
Note:
- This is a SkyGuard specialization.
- Titan uses flat addressing for bounded worlds (§10.1).
...
## 11. Streaming residency state machine (Titan)

Titan streaming requires explicit chunk states:
...
## 12. TileMap chunking (Titan-only) with sub-pages
...
### 12.4 TileMap dense query contract (Titan)
...
SkyGuard uses atlas path (`layerIndex = 0`) without Titan shaders.  
Titan may adopt TextureArrays later without refactoring mesh builders.

---

## 14. Zoom LOD & proxy rendering (Titan-only)
...
## 16. Chunk data IO (Titan-only): non-blocking, mapping-capable backend
...
Titan requires the performance characteristics; exact backend is platform-dependent.

---

## 17. Cross-system invalidation hooks (Titan)
...
- stack membership changes → mark affected entities dirty (Titan gameplay decision).
...
Titan determinism checks (Debug/Profile):
...
## 21. Memory ceilings (Titan)

Titan must define explicit ceilings and enforce them with metrics/logging:
...
```cpp
SpatialIndex::Config skyguardDefaults {
    .chunkSizeTiles = 0,
    .chunkSizeWorld = 4096,
    .cellSizeWorld = 512,
    .useFlatArray = false,
...
SpatialIndex::Config titanDefaults {
    .chunkSizeTiles = 128,
...
- Two query modes: fast unordered (render) + deterministic (Titan simulation).
- Deterministic update order in Titan (dirty set sorted by StableID).
...
- Titan streaming has explicit residency states;
...
- Titan: flat chunk addressing with payload pooling.
- SkyGuard: bounded-capacity sparse/ring storage.
- Internal vertex format carries `pageIndex/layerIndex`; SkyGuard can ignore; Titan can adopt TextureArrays later.
- Zoom-out proxy rendering is required for Titan; interface now, implementation later.
- Titan chunk IO is non-blocking and mapping-capable (or equivalent).
