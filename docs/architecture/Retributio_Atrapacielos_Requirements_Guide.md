...
**Prototype:** Atrapacielos (aerial sim for validation)  
...
Retributio is a high-performance 2D engine for large-scale turn-based grand strategy games, targeting flagship 4X Auctoritas quality. The engine is flexible enough for side projects (platformers, small RPGs) for rapid development and funding, but is optimized and architected specifically for 4X grand strategy with 100K+ entities, complex AI, and scientific simulation.

Atrapacielos is the first prototype: a small aerial simulator serving as a validation lab for ECS architecture, resource management, performance, and UI systems. Atrapacielos will never reach 50K entities—it uses stress tests with virtual entities to validate scalability for the flagship 4X.
...
`core/` is engine-only, game-agnostic, and reusable across projects. `game/*` (e.g., `game::atrapacielos`) is game-specific and never pollutes core. Clear boundaries enable engine reuse and prevent coupling.
...
**Structure:**
- `engine/include/` — Engine headers
- `engine/src/` — Engine implementation
- `games/atrapacielos/include/` — Game-specific headers
- `games/atrapacielos/src/` — Game-specific implementation
- `engine/assets/` — Engine assets (fonts, core configs)
- `games/atrapacielos/assets/` — Game assets (textures, sounds, game configs)
...
**Canonical mapping:** `core/log/*` → Engine; `core/config/*` → Config; `core/resources/*` → Resources; `core/ecs/*` → ECS; `core/ui/*` → UI; `core/time/*` → Engine (service init) or Debug/Performance (profiling); `core/utils/json/*` → Config; `*` (config) → Config for JSON errors, Gameplay for game outcomes.
